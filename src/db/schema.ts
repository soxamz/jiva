...
export const userRole = pgEnum('user_role', ['patient', 'doctor', 'responder', 'admin']);
export const userStatus = pgEnum('user_status', ['active', 'deceased']);
export const documentStatus = pgEnum('document_status', ['processing', 'processed', 'failed']);
export const documentType = pgEnum('document_type', ['lab', 'rx', 'note', 'discharge', 'other']);
export const consentStatus = pgEnum('consent_status', ['active', 'revoked', 'expired']);
...
export const structuredData = pgTable('structured_data', {
  ...
  abnormalValues: jsonb('abnormal_values')
    .$type<Array<{ label: string; value: string; severity: 'low' | 'medium' | 'high' }>>()
    .notNull()
    .default(sql`'[]'::jsonb`),
...
export const consents = pgTable(
  'consents',
  {
    ...
    patientId: uuid('patient_id')
      .notNull()
      .references(() => users.id, { onDelete: 'cascade' }),
    granteeId: uuid('grantee_id').references(() => users.id, { onDelete: 'set null' }),
...
export const auditLogs = pgTable(
  'audit_logs',
  {
    ...
    ipAddress: varchar('ip_address', { length: 80 }).notNull().default('demo-local'),
